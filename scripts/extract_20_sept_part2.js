...
const ISSUES_CONFIG = [
    {
        name: "20 September Part 2",
        year: 2020,
        month: "september",
        issueOrder: 2,
        volume: "15(2)",
        mergedPdfUrl: "https://res.cloudinary.com/dyzvvmpsq/raw/upload/v1779278992/spectra_issues/2_1779278986545.pdf",
        tocPdfUrl: null
    }
];
...
async function run() {
    ...
    for (const config of issuesToProcess) {
        ...
        // 3. Parse TOC
        let parsedArticles;
        if (config.name.startsWith("20 September")) {
        ...
        } else if (config.name === "20 September Part 2") {
            offset = -208;
            console.log('Using -208 offset for part 2 directly scanned pages.');
        } else {
